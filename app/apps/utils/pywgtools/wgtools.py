...
def genpsk(*, _wg: Iterable[str] = WG) -> str:
    ...
    return check_output([*_wg, 'genpsk'], text=True, shell=True).strip()
...
def show(
    ...
    if interface == 'all':
        text = check_output([*_wg, 'show', 'all'], text=True, shell=True).strip()
        return parse_interfaces(text, raw=raw, json=json)

    if interface == 'interfaces':
        text = check_output([*_wg, 'show', 'interfaces'], text=True, shell=True).strip()
        return text.split()

    text = check_output([*_wg, 'show', interface], text=True, shell=True).strip()
...
def clear_peers(interface: str, *, _wg: Iterable[str] = WG) -> None:
    ...
    if peers:
        set_interface(interface, peers=peers, _wg=_wg)
