...
from apps.utils.pyopnsense import exceptions
...
class OPNClient(object):
    ...
    def _post_json(self, endpoint, payload, raw=False):
        if self.verify_cert is False:
            urllib3.disable_warnings()

        req_url = '{}/{}'.format(self.base_url, endpoint)
        response = requests.post(req_url, json=payload, verify=self.verify_cert,
                                 auth=(self.api_key, self.api_secret),
                                 timeout=DEFAULT_TIMEOUT)
        return self._process_response(response, raw)

    def _generate_endpoint(self, command, parameters=None):
        return f'{self.module}/{self.controller}/{command}' + (f'/{parameters}' if parameters else '')
